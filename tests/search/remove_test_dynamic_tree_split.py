...
#         tree = ParallelTree(3, '', 0, 0.0, OpenInterval(-1, 1), move_sorter, play)
...
class DynamicTreeSearchTest(unittest.TestCase):
    def endgame_test(self, index: int):
        result = DynamicTreeSearch(PrincipalVariation(), 3).eval(endgame[index].pos)
        self.assertEqual(result, endgame[index].score)
